...
import { getJobsByLocation, getJobsByPayRange } from '@/actions/server'
...
const JOBS = () => {
  ...
  const handleSalaryFilter = async (range) => {
    ...
    const data = await getJobsByPayRange(min, max)
    setResults(data)
  }

  const handleLocationFilter = async (loc) => {
    setFilterType('location')
    const data = await getJobsByLocation(loc)
    setResults(data)
  }
  ...
  <option className="bg-white text-black" value="Remote">Remote</option>
      ...
      {!filterType && <InternshipCard type="JOB" />}
